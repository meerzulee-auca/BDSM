...
GO
    ...
    CREATE TABLE dbo.Floors (
        ...
        BuildingID VARCHAR(100) NOT NULL CONSTRAINT FK_Building FOREIGN KEY REFERENCES dbo.Building(Address),
        LEVEL INT NOT NULL,
...
GO
    ...
    CREATE TABLE dbo.WorkingRoom (
        ...
        FloorID bigint NOT NULL CONSTRAINT FK_Floors FOREIGN KEY REFERENCES dbo.Floors(ID),
...
GO
    -- Garniture types
    CREATE TABLE dbo.Garniture (
        ID Bigint NOT NULL CONSTRAINT PK_Garniture PRIMARY KEY CLUSTERED,
        Title VARCHAR(90),
        CostPerUnit MONEY NOT NULL
    );

GO
    -- Garnitures in the room
    CREATE TABLE dbo.ConditionsInWorkingSpaces (
        ID Bigint NOT NULL CONSTRAINT PK_ConditionsInWorkingSpaces PRIMARY KEY CLUSTERED,
        FurnitureType bigint NOT NULL CONSTRAINT FK_Garniture FOREIGN KEY REFERENCES dbo.Garniture(ID),
        RoomID bigint NOT NULL CONSTRAINT FK_WorkingRoom FOREIGN KEY REFERENCES dbo.WorkingRoom(ID),
        Amount Int NOT NULL
    );
...
GO
    ...
    -- Drop the table if it already exists
    IF OBJECT_ID('dbo.MarketSectors', 'U') IS NOT NULL DROP TABLE dbo.MarketSectors
GO
    -- Create the table in the specified schema
...
GO
    ...
    -- Drop the table if it already exists
    IF OBJECT_ID('dbo.CompaniesSectors', 'U') IS NOT NULL DROP TABLE dbo.CompaniesSectors
GO
    -- Create the table in the specified schema
...
GO
    ...
    CREATE TABLE dbo.Cards (
        ID BIGSERIAL NOT NULL PRIMARY KEY,
        PersonID bigint NOT NULL CONSTRAINT FK_Cards_Persons FOREIGN KEY REFERENCES dbo.Persons(ID),
...
GO
    ...
    -- Drop the table if it already exists
    IF OBJECT_ID('dbo.ClientEmployees', 'U') IS NOT NULL DROP TABLE dbo.ClientEmployees
GO
    -- Create the table in the specified schema
    CREATE TABLE dbo.ClientEmployees (
        ID BIGSERIAL NOT NULL PRIMARY KEY,
        CompanyID bigint NOT NULL CONSTRAINT FK_ClientEmployees_Companies FOREIGN KEY REFERENCES dbo.Companies(ID),
        PersonID bigint NOT NULL CONSTRAINT FK_ClientEmployees_Persons FOREIGN KEY REFERENCES dbo.Persons(ID)
    );
...
GO
    ...
    -- Drop the table if it already exists
    IF OBJECT_ID('dbo.Parkings', 'U') IS NOT NULL DROP TABLE dbo.Parkings
GO
    -- Create the table in the specified schema
    CREATE TABLE dbo.Parkings (
        ID BIGSERIAL NOT NULL PRIMARY KEY,
        -- primary key column
        BuildingID varchar(100) NOT NULL CONSTRAINT FK_Parkings_Buildings FOREIGN KEY REFERENCES dbo.Buildings(Address),
        Capacity int NOT NULL,
        IsUnderground boolean NOT NULL -- specify more columns here
    );
